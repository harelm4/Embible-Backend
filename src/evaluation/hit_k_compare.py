...
def hit_at_k_eval(model: Model, data: str, domain: str, mask_precent):
    ...
    t1 = time.time()
    if(domain=="word"):
      res=calculate_word_hit_at_k(k, model, input_data,domain)
    else:
        res = calculate_hit_at_k(k, model, input_data,domain)
...
for mask_precent in data_masked:
    ...
    # except Exception as e:
    #     print(e)
    hit_at_k_eval(StandardModel(f'Embible/super-Mbert')
                  , char_token_with_masked_spaces
                  , 'word'
                  , mask_precent
                  )
...
res_df.to_csv(csv_location + 'hit_at_k_compare_result_AlephBertGimmel.csv')
